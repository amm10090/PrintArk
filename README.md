...
4. 双击打开 `PrintArk.app`。
5. 首次打开后，进入「版本信息」页面，在「服务信息」中点击「安装本机证书」。该证书由 PrintArk 在本机独立生成，用于 `wss://localhost:13529/`，不依赖菜鸟打印软件。
...
从 v1.1.13 开始，冷启动、重启和睡眠唤醒均由 PrintArk 自行完成本机端口监听与远端兼容握手，不会启动官方菜鸟打印组件。`localhost` TLS 证书也由 PrintArk 按设备生成并保存到自己的 Application Support 目录。
